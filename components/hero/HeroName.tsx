      ...
      >
        ...
        */}
        <meshPhysicalMaterial
          // GREYISH AND TRANSPARENT, not transmissive. Transmission refracts
          // the SCENE buffer, and the only thing behind this mesh in the scene
          // is empty space — the particle field is a DOM canvas underneath a
          // transparent WebGL canvas, which WebGL cannot sample. A transmissive
          // material here therefore refracted nothing and rendered as a dark
          // slab, which is exactly what it did when it was tried.
          //
          // Plain alpha blending is the correct mechanism for this stack: the
          // canvas is `alpha: true`, so an `opacity` below 1 lets the real
          // particle field through by compositing rather than by simulation.
          // Cheaper, and it actually shows what is behind it.
          color="#c8ced4"
          transparent
          opacity={0.42}
          roughness={0.16}
          ...
          // letterforms keep a crisp lit edge at 42% body opacity.
          clearcoat={1}
          clearcoatRoughness={0.1}
          ior={1.45}
          reflectivity={0.6}
          // depthWrite STAYS ON despite being transparent. Off, the extrusion
          // self-sorts and you see through the front faces into the back ones,
          // which reads as mush rather than as a solid pane of glass.
          depthWrite
          // THE STENCIL WRITE. Marks every pixel the glyphs cover with a 1 so
          // `LiquidBlob` can test against it. This is what contains the liquid
          // inside the letterforms now that there is no SVG mask.
          stencilWrite
          stencilRef={1}
