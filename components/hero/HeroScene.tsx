...
import { Box3, Vector3, type PerspectiveCamera as ThreePerspectiveCamera } from "three";
  ...
  return (
    <>
      ...
      */}
      ...
      {/*
        UNGATED, unlike the particle field it replaces. That field needed the
        wordmark's measured bbox to carve an exclusion shell around the
        letters, so it had to wait for the geometry. The grid needs no such
        thing: it is a flat plane three units behind the type, and the type
        occludes and refracts it by simply being in front. Waiting on
        `measured` would leave the hero empty for the first frames for no
        reason.
      */}
      <HeroGrid />
