 ...
 * at segment level `/`, the same level as `app/(site)/page.tsx`. `(.)` means
 * "intercept a route at my own level", so `(.)projects/[slug]` intercepts
 ...
 *   - Click a card on `/` → the URL becomes `/projects/<slug>`, this renders as
 *     an overlay, and `/` stays mounted behind it (which is what gives the
 *     morph something to morph FROM).
 ...
 * boundary — a 404 rendered BESIDE a still-visible homepage. Incoherent.
 *
